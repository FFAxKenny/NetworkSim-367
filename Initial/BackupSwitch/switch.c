...
/*
 * The switch object is meant to be an intermediate transmitter for data being
 *  transmitted to different hosts.  It needs to run similar to a host, but
 *  can operate independently of a managing application.
 * It will be launched at the same time the hosts are launched, by the
 *  manager service, and then should run in the background until a host tries
 *  to transmit data.  The data will be received by the switch and then echoed
 *  to all other hosts except the source.
 * Requests coming into the switch are stored in a queue and dealt with on a
 *  first come, first served basis (queue style).
 * The switch should close at the same time as all other processes, when the
 *  managing application closes.  Otherwise it will remain active and open.
 * Changes will need to be made to the managing program to accomodate the
 *  differences in the mode of operation.
 *
 *
 */
 ...
 *
 * -Will most likely be built using mostly host.c code pieces
 *
 *  -launches with manager
 *  -takes in transmit feed from all hosts
 *  -is able to broadcast to all hosts
 *  -is not accessible to the manager (at least not by casual users)
 *  -MAIN LOOP
 *  --checks to see if any new items are to be added to queue.
 *  --broadcasts an item from top if there are any in queue.
 *  --sleeps for 10ms
 *
 *
 *
