...
do {
   ...
   if (strcmp(word, "DISPLAY")==0)
	{
		 manDisplayReplyMsg(reply);
	}
   else if (strcmp(word, "GetHostStateAck") == 0)
	{
		manDisplayHostState(reply);
	}
} while(length <= 1);
}
...
/* Ask the user to enter the destination address */
...
do {
   printf("Enter host ID number (range %d to %d): ", 
	switcount,switcount+hostcount-1);
   scanf("%d",&newnumber);
   if (newnumber >= switcount && newnumber < switcount+hostcount) break;
...
currhost = switcount;      /* Manager is initially connected to first host */
...
while(1) {
   ...
   if (cmd == 'q') return;
	...
	printf("Reply Recieved");
      ...
      manDisplayHosts(currhost, hostcount+switcount);
